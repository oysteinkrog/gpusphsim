...
#
# CUDA float3 is 12 bytes (3 x float32, no trailing padding).
# Five float3 fields = 60 bytes total.
# ---------------------------------------------------------------------------

GRID_PARAMS_DTYPE = np.dtype(
    [
        ("grid_size", np.float32, (3,)),
        ("grid_min", np.float32, (3,)),
        ("grid_max", np.float32, (3,)),
        ("grid_res", np.float32, (3,)),
        ("grid_delta", np.float32, (3,)),
    ],
    align=False,  # CUDA float3 has no trailing pad, so no alignment
)
...
def build_grid_params() -> np.ndarray:
    ...
    params[0]["grid_res"] = GRID_RES.astype(np.float32)
